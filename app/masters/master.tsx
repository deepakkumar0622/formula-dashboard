...
export default function MastersPage() {
  ...
  const [formType, setFormType] = useState<"ingredient" | "formula">(
    "ingredient",
  );
  const formula = {
    name: "",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-2">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium cursor-pointer ${
              activeTab === "formulas"
                ? "border borger-gray-900 text-gray-900"
                : ""
            }`}
          >
            Formulas (6)
      ...
      <button
        onClick={() => {
          setShowTabulator(!showTabulator);
        }}
        className="ml-4 text-sm bg-gray-900 text-white px-2 py-1 rounded"
      >
        Toggle View
      </button>
    </div>
